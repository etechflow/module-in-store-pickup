...
/**
 * Portal license validator for ETechFlow_InStorePickup.
 *
 *   isValid() priority:
 *     1. revoked = 1                       → false (portal revoke wins even in dev)
 *     2. production_environment = No       → true (dev bypass — toggle hardcoded off)
 ...
 *   fabricated either.
 *
 *   Portal-first ordering is what makes IP-revocation work: when the admin
...
class LicenseValidator
{
    ...
    public const XML_PATH_LICENSE_KEY            = 'etechflow_instorepickup/license/license_key';
    public const XML_PATH_PORTAL_URL             = 'etechflow_instorepickup/license/portal_url';
    public const XML_PATH_PORTAL_API_URL         = 'etechflow_instorepickup/license/portal_api_url';
    public const XML_PATH_PRODUCTION_ENVIRONMENT = 'etechflow_instorepickup/license/production_environment';
    ...
    // portal — kept for admin controllers that build the browser URL from it.
    ...
    private const CACHE_TAG    = 'ETECHFLOW_ISP';
    private const CACHE_PREFIX = 'etf_isp_lic_';
    ...
    public function isValid(): bool
    {
        ...
        // own domain, so they were removed. The former client-settable grace
        // (issued_key/issued_at/ip_blocked) was removed for the same reason:
        // every input lived in admin config the customer controls. Offline
        // grace now comes only from a cached genuine portal success, which the
        // customer cannot fabricate.
        if (!str_starts_with($configuredKey, 'SP-')) {
            return false;
        }
    ...
    private function validateViaPortal(string $host, string $licenseKey): ?bool
    {
        ...
        try {
            ...
            $this->curl->addHeader('User-Agent', 'ETechFlow-ISP/2.1');
            $this->curl->get($url);
    ...
    /**
     * Browser-facing portal URL, kept for the admin license controllers that
     * derive their checkout/gate base from it. Never consulted by isValid().
     */
    ...
    private function isExplicitlyRevoked(): bool
    {
        return (string) $this->scopeConfig->getValue(
            'etechflow_instorepickup/license/revoked',
