...
/**
 * Mass-delete handler from the Stores grid action dropdown.
 */
        ...
        try {
            ...
            foreach ($collection as $store) {
                try {
                    ...
                    $this->logger->error('ETechFlow_InStorePickup: mass-delete row failed.', [
                        'store_id'  => $store->getStoreId(),
                        'exception' => $e->getMessage(),
                    ]);
                }
            }
            $this->messageManager->addSuccessMessage(__('%1 store(s) deleted.', $deleted));
        } catch (\Throwable $e) {
